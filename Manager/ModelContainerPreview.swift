...
struct ModelContainerPreview<Content: View>: View {
    ...
    ///             }
    ///         } modelContainer: {
    ///             let schema = Schema([Trip.self, BucketListItem.self, LivingAccommodation.self, InternalError.self])
    ///             let configuration = ModelConfiguration(inMemory: true)
    ///             let container = try! ModelContainer(for: schema, configurations: [configuration])
    ///             let sampleData: [any PersistentModel] = [
    ///                 Trip.preview, BucketListItem.preview, LivingAccommodation.preview,, InternalError.preview
    ///             ]
    ///             Task { @MainActor in
    ///                 sampleData.forEach {
